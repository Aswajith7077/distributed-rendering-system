...
# Live snapshot for every connected slave: { node_id: metrics_dict }
_slave_registry: dict[str, dict[str, Any]] = {}

# All currently connected frontend WebSocket clients
...
def _build_aggregate() -> dict[str, Any]:
    ...
    """
    nodes = list(_slave_registry.values())
...
async def _persist_to_redis(node_id: str, metrics: dict[str, Any]) -> None:
    """Write the slave snapshot to Redis with a TTL."""
    if _redis is None:
        return
    try:
        key = f"health:node:{node_id}"
        await _redis.set(key, json.dumps(metrics), ex=SLAVE_TTL)
        await _redis.sadd("health:active_nodes", node_id)
...
async def _load_from_redis() -> None:
    ...
    if _redis is None:
        return
    try:
        node_ids: set[str] = await _redis.smembers("health:active_nodes")
        for node_id in node_ids:
            raw = await _redis.get(f"health:node:{node_id}")
            if raw:
                _slave_registry[node_id] = json.loads(raw)
        log.info("Hydrated %d node(s) from Redis on startup.", len(_slave_registry))
    except Exception as exc:
        log.warning("Could not hydrate from Redis: %s", exc)


# ── background task: heartbeat broadcaster ────────────────────────────────────
...
async def _heartbeat_task() -> None:
    ...
    while True:
        ...
        if _frontend_clients:
            await _broadcast(_build_aggregate())


# ── /ws/slave — inbound from slave nodes ──────────────────────────────────────
...
async def slave_endpoint(websocket: WebSocket) -> None:
    ...
    try:
        async for raw in websocket.iter_text():
            ...
            asyncio.create_task(_persist_to_redis(node_id, data))
            asyncio.create_task(_broadcast(_build_aggregate()))
...
# ── /ws/metrics — outbound to frontend clients ────────────────────────────────
...
async def metrics_endpoint(websocket: WebSocket) -> None:
    ...
    try:
        await websocket.send_text(json.dumps(_build_aggregate()))
    except Exception:
...
# ── REST fallback: GET /health/nodes ──────────────────────────────────────────
...
async def get_benchmark():
    ...
    completed_jobs = [j for j in jobs if j.get("status") == "completed" and "completed_at" in j]
    ...
    if t1_jobs:
         ...
         base_frame_time = (best_job["duration"] * best_job["workers"] * 0.9) / best_job["frames"]
    ...
    for n in range(1, max_workers + 3):
        ...
        theoretical_data.append({
            "workers": n,
            "theoretical_speedup": round(s_n, 2)
        })
